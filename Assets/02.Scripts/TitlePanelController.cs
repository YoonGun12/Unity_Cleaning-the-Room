...
public class TitlePanelController : MonoBehaviour
{
    ...
    {
        float startY = 700f; // 시작 위치 (화면 위)
        float endY = 0f; // 최종 위치
        float bounceHeight = 100f; // 튀는 높이
        float duration = 0.5f; // 기본 내려오는 시간
        float bounceDuration = 0.5f; // 튀는 시간
        float delayBetweenLetters = 0.4f; // 각 글자의 시작 시간 간격
